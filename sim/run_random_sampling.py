...
def get_number_of_sims(repeats, dflist, df_cols):
    lenrun = len(dflist)
    return int(repeats*(2 - lenrun/TIME_LIMIT))
...
def get_init_condition_from_df(dflist, df_cols, starts):
    ...
    ids = [int(np.mean(df.index[df['node'] == feature].tolist())) for feature in unique_node_ids]
    # times = [df. ]
        ...
        for sites in site_configs:
            for mTime in mTimes:
                for agents in agent_configs:
                    qualities = np.array([[0.191, 0.59, 0.51, 0.564]])#np.array([[0.913, 0.196, 0.388, 0.893]]) #get_valid_qualities(sites, sims_per_config)
                    init_configs = []
                    # print('got qualities')
                        ...
                        for distance in distances:
                            for sim_dist_iter in range(0,sims_per_distance):
                                ...
                                for agent_init in all_agent_inits:
                                    w_init = [sites, qual, poses, agents, agent_init, mTime, 0] 
...
if __name__ == '__main__':
    ...
    fold_name = 'AAAI/data/lots_of_node_samples/train_new_random_sample_128/'
