...
def process_file(fileName, site_conv, time_conv, entry, folder, folder_graph1, folder_graph2):
    ...
    sorted_quals = np.sort(quals)
        ...
        # if len(G.nodes) < 50:
        #     continue
    if sorted_quals[-1] - sorted_quals[-2] < 0.5: #np.min(quals) < 0.2 or np.max(quals) - np.min(quals) > 0.8:# and np.max(quals) - np.min(quals) > 0.3:
    ...
    folder_graph = './RS/single_graph_things/finishedall/' if success_now != 0.0 else './RS/single_graph_things/unfinishedall/'
    pdb.set_trace()
