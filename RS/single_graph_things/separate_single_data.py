...
fil2 = open('./RS/single_graph_things/MULTIPLEalltime.pickle', 'rb')
metatime = pickle.load(fil2)
fil2.close()
fil2 = open('./RS/single_graph_things/MULTIPLEallsucc.pickle', 'rb')
...
folder_graph = './RS/single_graph_things/finished_with_labels_test/'
new = './RS/single_graph_things/onlyMoreNodesGraphs_train/'
files = os.listdir(folder_graph)
files = [file for file in files if file.endswith('.pickle') and file.startswith('(0.934, 0.973, 0.131, 0.546)((200.0, 0.0), (0.0, 200.0), (-200.0, 0.0), (-0.0, -200.0))10')]
maxquals = []
minquals = []
nSites = []
siteDist = []
...
for file in files[:-1]:
    # if file.startswith('(0.934, 0.973, 0.131, 0.546)((200.0, 0.0), (0.0, 200.0), (-200.0, 0.0), (-0.0, -200.0))10'):
        ...
        for node in G.nodes(data=True):
            entry = round_function(node[1]['x'], 3)
