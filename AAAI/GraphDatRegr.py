...
class GraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     print(node)
        # pdb.set_trace()
        node_features = torch.tensor([node[1]['xold'] for node in G.nodes(data=True)], dtype=torch.float)
        ...
        if self.flag == 'train':
            if self.timesucc == 'time':
                ...
                # timesucc = torch.tensor([node[1]['times_conved'] - node[1]['time'] for node in G.nodes(data=True)], dtype=torch.float)
            else:
            ...
            if self.timesucc == 'time':
                # timesucc = torch.tensor([node[1]['AvgTime'] for node in G.nodes(data=True)], dtype=torch.float)
                timesucc = torch.tensor([node[1]['time'] for node in G.nodes(data=True)], dtype=torch.float)
            else:
                timesucc = torch.tensor([node[1]['AvgSucc'] for node in G.nodes(data=True)], dtype=torch.float)
