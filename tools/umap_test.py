...
reducer = umap.UMAP(n_components=3, n_neighbors=15, verbose=1, n_jobs=-1)# random_state=42)
d3Trans = reducer.fit_transform(dat)
...
fil_tsne3 = open(folder +'combined_umap.pickle', 'wb')
...
dict_3d = {'x': list(d3Trans[:,0]), 'y': list(d3Trans[:,1]), 'z': list(d3Trans[:,2]), 'colors': allCols}
